...
typedef struct {
    ...
    // return the exact blocknum(no need to be continuos) that can be used for readblock()
    //of the block that has the data
    // direct_ptrs start from 0, 1, 2,..., 29, one by one, so it's blockindex
} inode_t;
